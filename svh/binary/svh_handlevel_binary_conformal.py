...
# -*- coding: utf-8 -*-
"""
Hand-level SvH Binary Classification (0 vs >0) with a ViT backbone.
...
- ✅ Keeps the same logic: build hand-level dataset from joint CSV, patient-level split, AMP, split conformal

Task definition (binary):
- erosion: require exactly 13 joints with valid erosion in the image; label = (sum > 0)
- jsn: require exactly 9 joints with valid jsn in the image; label = (sum > 0)

Selection:
- Save best by VAL AUC (to avoid test leakage)
- Print conformal stats on VAL(calib-self) and TEST every epoch (or at least for best epoch)
"""
...
def parse_args():
    ...
    p.add_argument("--task", type=str, default=os.environ.get("TASK", "jsn"),
                   choices=["erosion", "jsn"], help="Binary task: 0 vs >0 based on hand-level sum.")

    # expected joints per hand
    p.add_argument("--expected_joints_erosion", type=int, default=int(os.environ.get("EXPECTED_JOINTS_EROSION", "13")))
    p.add_argument("--expected_joints_jsn", type=int, default=int(os.environ.get("EXPECTED_JOINTS_JSN", "9")))
    ...
    p.add_argument("--whole_img_root", type=str, default=os.environ.get("WHOLE_IMG_ROOT", ""),
                   help="Directory containing whole-hand images referenced by inferred hand_image names.")
    ...
    # io
    p.add_argument("--output_dir", type=str, default=os.environ.get("OUTPUT_DIR", "./outputs/svh_hand_binary"),
...
# =========================
def build_hand_level_df(csv_path: str, whole_img_root: Path, task: str,
                        expected_joints_erosion: int, expected_joints_jsn: int) -> pd.DataFrame:
    """
    Build hand-level df from joint-level csv.

    Keep images satisfying:
    - Whole image has exactly expected_joints valid joints (for the task)
    - For each (patient_id, timepoint, side, hand_image) group, it has exactly expected_joints joints
    - Image file exists in whole_img_root
    """
    ...
    log(f"Unique hand_image candidates in CSV (task={task}): {df_valid['hand_image'].nunique()}")
    ...
    log(f"[{task}] whole images with exactly {expected_joints} valid joints: {len(single_img)}")
    ...
    log(
        f"Built hand-level df (task={task}): N={len(df_hand)} "
        f"with expected_joints={expected_joints} and existing whole images."
    )
    return df_hand
...
def main():
    ...
    task = args.task
    expected_e = args.expected_joints_erosion
    expected_j = args.expected_joints_jsn
    ...
    # ---- build hand-level df ----
    df_hand = build_hand_level_df(
        csv_path=args.csv_path,
        whole_img_root=whole_root,
        task=task,
        expected_joints_erosion=expected_e,
        expected_joints_jsn=expected_j,
    )
    if df_hand.empty:
        raise RuntimeError(f"No valid hand-level samples found for task={task}.")

    # ---- split ----
    df_tr, df_val, df_te = patient_level_split_3way(
        df_hand, train_ratio=args.train_ratio, val_ratio=args.val_ratio, seed=args.seed
    )
    log(
    ...
    # ---- datasets & loaders ----
    train_ds = HandImageDataset(df_tr, whole_root, train_tf)
    val_ds = HandImageDataset(df_val, whole_root, eval_tf)
    test_ds = HandImageDataset(df_te, whole_root, eval_tf)
    ...
    for ep in range(0, args.epochs + 1):
        ...
        if np.isfinite(val_auc) and val_auc > best_val_auc:
            ...
            save_path = os.path.join(args.output_dir, f"hand_{task}_binary_ep{ep}_valauc{best_val_auc:.4f}.pt")
...
# Example:
# python3 svh_handlevel_binary_conformal.py \
#   --task jsn \
#   --csv_path /path/to/RA_joint_score_raw.csv \
#   --whole_img_root /path/to/whole_hand_images \
#   --fm_ckpt_path /path/to/foundation_ckpt.pt \
#   --output_dir ./outputs/svh_hand_binary \
#   --use_conformal
