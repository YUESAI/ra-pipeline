...
# -*- coding: utf-8 -*-
"""
Hand-level SvH Binary Classification on CATCH dataset with DINOv3 FM backbone

Task (binary):
- TASK="erosion":
    * One hand image must have exactly 13 joints with valid Erosion scores
    * Erosion_sum = sum of 13 joint Erosion scores (0~65)
    * Binary label: Erosion_sum == 0 -> 0, Erosion_sum > 0 -> 1

- TASK="jsn":
    * One hand image must have exactly 9 joints with valid JSN scores
    * JSN_sum = sum of 9 joint JSN scores (0~36)
    * Binary label: JSN_sum == 0 -> 0, JSN_sum > 0 -> 1

Model:
- Encoder: DINOv3 ViT-B/16 student (AutoModel) + ema_state
- Head: mean pooling -> LN -> Linear -> one logit (two heads: erosion/jsn)
- Loss: masked BCEWithLogits + pos_weight
- Metrics: AUC / ACC / AUPR / F1
- Split: patient-level 0.8/0.1/0.1 (train/val/test), seed=3407
- Conformal prediction: split conformal (calibration = val split)
- Checkpoint: save best by VAL AUC (avoid test leakage). When saving, also print conformal stats on VAL/TEST.
"""
...
# -------- Foundation Model checkpoint (ema_state) --------
...
MODEL_SAVE_DIR = "/home/UWO/ylong66/data/RA/LLM/ckpt/train/multi_expert_ema_handlevel"
...
def extract_hand_image_name(file_name: str) -> str:
    """
    Infer whole-hand image name from joint image name.
    Pattern:
        <base>_<side>_<joint_type>_<index>.tif
    Example:
        08-2434_C_hands_right_MCP_3.tif      -> 08-2434_C_hands.tif
        10-1428_A_HandRight_right_MCP_3.tif  -> 10-1428_A_HandRight.tif
    """
...
# =========================
# Model (LN + Linear head) —— logits
# =========================
class HandSVHModel(nn.Module):
...
def conformal_predict_sets(test_logits: np.ndarray,
    ...
    cal = cal_scores.reshape(1, -1)   # [1, n_cal]
    t0c = t0.reshape(-1, 1)
    t1c = t1.reshape(-1, 1)
...
def eval_loader_collect(model, loader, target_idx: int):
    """
    Collect logits + targets for the current task.
    Returns:
      pred_logits [N]
      tgt_scores  [N] (hand-level sums, -1 invalid)
    """
    model.eval()
    pred_list, tgt_list = [], []
...
# =========================
# Build hand-level dataframe (per task)
# =========================
def build_hand_level_df(task: str) -> pd.DataFrame:
    """
    Build hand-level dataframe. Keep truly unilateral images by definition:
      - For current task, the whole image has exactly expected_joints valid joints.
      - And for that side/hand in that image, it also has exactly expected_joints joints.
    """
    ...
    log(f"Unique hand_image candidates from CSV for {task}: {df_valid['hand_image'].nunique()}")
    ...
    single_img = img_counts[img_counts["num_joints_image"] == expected_joints]
    log(f"[{task}] whole images with exactly {expected_joints} valid joints (single-hand images): {len(single_img)}")

    single_img_keys = set(
        (row["patient_id"], row["timepoint"], row["hand_image"])
        for _, row in single_img.iterrows()
    )
    ...
    log(
        f"Built hand-level DataFrame for task={task}: {len(df_hand)} hands "
        f"with exactly {expected_joints} joints AND whole images that have exactly {expected_joints} joints total."
    )
    return df_hand
...
# =========================
def main():
    log(f"Running hand-level task = {TASK} (Binary 0 vs >0)")
    ...
    # ---- build hand-level df ----
    df_hand = build_hand_level_df(task=TASK)
    if df_hand.empty:
        raise RuntimeError(f"No valid single-hand images found for task={TASK}.")
    ...
    df_tr, df_val, df_te = patient_level_split_3way(
        df_hand, train_ratio=TRAIN_RATIO, val_ratio=VAL_RATIO, seed=SPLIT_SEED
    )
    log(
    ...
    val_ds   = HandImageDataset(df_val, WHOLE_IMG_ROOT, eval_tf)
    ...
    model = HandSVHModel(encoder).to(DEVICE)
    ...
    for ep in range(0, EPOCHS + 1):
        ...
        log_binary_metrics(f"Train {TASK.capitalize()} (hand-level)", tr_m)
        log_binary_metrics(f"Val   {TASK.capitalize()} (hand-level)", va_m)
        log_binary_metrics(f"Test  {TASK.capitalize()} (hand-level)", te_m)
        ...
        if np.isfinite(val_auc) and val_auc > best_val_auc:
            ...
            save_path = os.path.join(
                MODEL_SAVE_DIR,
                f"catch_hand_{TASK}_binary_dinov3_amp_ep{ep}_valauc{best_val_auc:.4f}.pt"
            )
            torch.save(model.state_dict(), save_path)
            log(f"✅ Saved best (by VAL {TASK.capitalize()} AUC) to {save_path}")

            # when saving, also re-print conformal on VAL/TEST (already printed above),
            # so here we just add a short marker
            if USE_CONFORMAL:
                log("🧷 (Saved best) Conformal stats above correspond to current best epoch.")
