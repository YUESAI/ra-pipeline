...
# -*- coding: utf-8 -*-
"""
CATCH Bilateral (two-hand) Erosion Regression with Warm-start from Bilateral Erosion Binary CKPT
...
- Gate head (compatible with binary): mean pooling -> LN -> Linear(1)  (head_e)
- Reg head: mean pooling -> LN -> MLP -> Linear(1)
- Prediction (bounded, non-negative):
    gate_prob = sigmoid(gate_logit)
    reg_val   = MAX_SUM_EROSION * sigmoid(reg_raw)   in [0,130]
    pred_soft = gate_prob * reg_val                  in [0,130]
...
Run:
  CUDA_VISIBLE_DEVICES=1 nohup python catch_svh_bilateral_erosion_gate_regression_conformal.py \
    > train_log/catch_svh_bilateral_erosion_gate_regression_conformal.log 2>&1 &
"""
...
# ✅ best bilateral erosion binary ckpt (encoder + head_e)
BINARY_CKPT_PATH = "/home/UWO/ylong66/data/RA/LLM/ckpt/train/multi_expert_ema_bilateral/catch_bilateral_erosion_binary_dinov3_amp.pt"
...
MODEL_SAVE_DIR = "/home/UWO/ylong66/data/RA/LLM/ckpt/train/multi_expert_ema_bilateral_erosion_regression"
...
def build_bilateral_df_erosion() -> pd.DataFrame:
    ...
    for (pid, tp, wimg), g in df_valid.groupby(group_cols):
        ...
        eros_sum = int(g["Erosion_clean"].sum())  # 0..130
        group_list.append({
            "patient_id": pid,
            "timepoint": tp,
            "whole_image": wimg,
    ...
    log(f"Built bilateral erosion DF: {len(df_bi)} images with exactly {EXPECTED_JOINTS_EROSION} valid joints")
    return df_bi


class BilateralErosionDataset(Dataset):
...
# =========================
# 4) Model (compatible with bilateral binary head_e)
# =========================
...
class BilateralErosionGateRegModel(nn.Module):
    """
    Keep head_e identical name/shape to bilateral binary (LN+Linear),
    so we can load binary ckpt (encoder + head_e) with strict=False.
    """
    def __init__(self, encoder: VisionEncoder):
        ...
        # ✅ same as bilateral binary head_e
    ...
    def forward(self, x) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        gate_logit = self.head_e(pooled).squeeze(1)
        reg_raw = self.reg_head(pooled).squeeze(1)
        return gate_logit, reg_raw


def warmstart_from_bilateral_erosion_binary_ckpt(model: BilateralErosionGateRegModel, binary_ckpt_path: str):
    sd = torch.load(binary_ckpt_path, map_location="cpu")
    if not isinstance(sd, dict):
        raise ValueError("Binary ckpt is not a state_dict dict.")
    missing, unexpected = model.load_state_dict(sd, strict=False)
    log(f"[WarmStart] Loaded from bilateral erosion binary ckpt: {binary_ckpt_path}")
    ...
    if "head_e.1.weight" in sd:
        ...
        log(f"  check ||head_e.W_loaded - head_e.W_src|| = {diff:.6f}")
...
# =========================
@torch.no_grad()
def predict_soft(model: BilateralErosionGateRegModel, loader: DataLoader) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    pred_soft in [0,130], y_true, gate_prob
    """
    model.eval()
    preds, ys, gates = [], [], []
    ...
    for img, y in loader:
        ...
        with autocast(enabled=(DEVICE == "cuda")):
            ...
            reg_val = float(MAX_SUM_EROSION) * torch.sigmoid(reg_raw)  # [0,130]
...
@torch.no_grad()
def eval_split(model: BilateralErosionGateRegModel, loader: DataLoader) -> Dict[str, float]:
    pred, y, _ = predict_soft(model, loader)
    return compute_metrics_reg(pred, y, max_label=MAX_SUM_EROSION)
...
# 8) Train
# =========================
def train_epoch(model: BilateralErosionGateRegModel,
                loader: DataLoader,
    ...
    for img, y in loader:
        ...
        with autocast(enabled=(DEVICE == "cuda")):
            gate_logit, reg_raw = model(img)
            gate_prob = torch.sigmoid(gate_logit)
            reg_val = float(MAX_SUM_EROSION) * torch.sigmoid(reg_raw)
            pred = gate_prob * reg_val
...
# =========================
def main():
    log("🚀 CATCH | BILATERAL | TASK=EROSION regression | warmstart bilateral erosion binary ckpt | pred=sigmoid(gate)*130*sigmoid(reg) | best by Val(SOFT) PCC | + Conformal")

    df_bi = build_bilateral_df_erosion()
    if df_bi.empty:
        raise RuntimeError("No valid bilateral erosion images found.")
    ...
    train_ds = BilateralErosionDataset(df_tr, WHOLE_IMG_ROOT, train_tf)
    val_ds = BilateralErosionDataset(df_va, WHOLE_IMG_ROOT, eval_tf)
    test_ds = BilateralErosionDataset(df_te, WHOLE_IMG_ROOT, eval_tf)
    ...
    model = BilateralErosionGateRegModel(encoder).to(DEVICE)

    # warmstart
    if BINARY_CKPT_PATH and os.path.exists(BINARY_CKPT_PATH):
        warmstart_from_bilateral_erosion_binary_ckpt(model, BINARY_CKPT_PATH)
    else:
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        if np.isfinite(curr_val_pcc) and curr_val_pcc > best_val_pcc:
            ...
            save_name = f"catch_bilateral_erosion_warmstart_reg_bestValSoftPCC{best_val_pcc:.4f}_ep{ep}.pt"
            best_ckpt_path = os.path.join(MODEL_SAVE_DIR, save_name)

            torch.save(
                {
                    "model": model.state_dict(),
                    "task": "bilateral_erosion_regression",
                    "epoch": ep,
                    ...
                    "config": {
                        ...
                        "PRED": "sigmoid(gate) * (130 * sigmoid(reg_raw))",
