...
# -*- coding: utf-8 -*-
"""
CATCH Hand-level SvH JSN Regression with Warm-start from Hand-level JSN Binary CKPT

Task (regression):
- One hand image must have exactly 9 joints with valid JSN scores (5 MCP + 4 PIP)
- JSN_sum = sum of 9 joint JSN scores (0~36)
- Predict JSN_sum (soft regression)

Model:
- Encoder: DINOv3 ViT-B/16 (AutoModel)
...
- Prediction (bounded, non-negative):
    ...
    reg_val   = MAX_SUM_JSN * sigmoid(reg_raw)     in [0,36]
    pred_soft = gate_prob * reg_val                in [0,36]
...
Run:
  CUDA_VISIBLE_DEVICES=2 nohup python catch_svh_hand_jsn_gate_regression_conformal.py \
    > train_log/catch_svh_hand_jsn_gate_regression_conformal.log 2>&1 &
"""
...
# expected joints per hand
EXPECTED_JOINTS_JSN = 9  # 5 MCP + 4 PIP
MAX_SUM_JSN = 36
...
# ✅ Best hand-level JSN binary ckpt (encoder + head_j)
BINARY_CKPT_PATH = "/home/UWO/ylong66/data/RA/LLM/ckpt/train/multi_expert_ema_handlevel/catch_hand_jsn_binary_dinov3_amp.pt"
...
MODEL_SAVE_DIR = "/home/UWO/ylong66/data/RA/LLM/ckpt/train/multi_expert_ema_handlevel_jsn_regression"
...
# two-stage
WARMUP_FREEZE_EPOCHS = 5          # freeze encoder+gate, train reg_head only
ENCODER_LR_MULT = 0.3             # after unfreeze
...
# =========================
# 3) Build hand-level df (JSN unilateral)
# =========================
...
def build_hand_level_df_jsn() -> pd.DataFrame:
    """
    Keep truly unilateral hand images for JSN:
      - whole image has exactly 9 valid JSN joints
      - and the side-level group also has exactly 9 joints
    """
    ...
    log(f"Unique hand_image candidates from CSV (jsn): {df_valid['hand_image'].nunique()}")
    ...
    single_img = img_counts[img_counts["num_joints_image"] == EXPECTED_JOINTS_JSN]
    log(f"[jsn] whole images with exactly {EXPECTED_JOINTS_JSN} valid joints: {len(single_img)}")
    ...
    for (pid, tp, sd, hand_img), g in df_valid.groupby(hand_group_cols):
        ...
        jsn_sum = int(g["JSN_clean"].sum())  # 0..36
        group_list.append({
            "patient_id": pid,
            "timepoint": tp,
            "side": sd,
    ...
    log(f"Built hand-level JSN DataFrame: {len(df_hand)} hands (exactly 9 joints, unilateral).")
    return df_hand


class HandJSNDataset(Dataset):
...
# =========================
# 4) Model (compatible with hand binary head_j)
# =========================
...
class HandJSNGateRegModel(nn.Module):
    """
    Keep head_j name identical to hand binary (LN + Linear),
    so binary ckpt loads encoder + head_j with strict=False.
    """
    ...
    def forward(self, x) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        gate_logit = self.head_j(pooled).squeeze(1)  # [B]
        reg_raw = self.reg_head(pooled).squeeze(1)   # [B]
        return gate_logit, reg_raw


def warmstart_from_hand_jsn_binary_ckpt(model: HandJSNGateRegModel, binary_ckpt_path: str):
    sd = torch.load(binary_ckpt_path, map_location="cpu")
    if not isinstance(sd, dict):
        raise ValueError("Binary ckpt is not a state_dict dict.")
    missing, unexpected = model.load_state_dict(sd, strict=False)
    log(f"[WarmStart] Loaded from hand JSN binary ckpt: {binary_ckpt_path}")
...
def compute_metrics_reg(y_pred: np.ndarray, y_true: np.ndarray, max_label: int) -> Dict[str, float]:
    ...
    # nz subset (optional, useful to monitor)
    nz = y_true_cls > 0
    if nz.sum() >= 2:
...
# =========================
@torch.no_grad()
def predict_soft(model: HandJSNGateRegModel, loader: DataLoader) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    pred_soft in [0,36], y_true, gate_prob
    """
    model.eval()
    preds, ys, gates = [], [], []
    ...
    for img, y in loader:
        ...
        with autocast(enabled=(DEVICE == "cuda")):
            ...
            reg_val = float(MAX_SUM_JSN) * torch.sigmoid(reg_raw)   # bounded [0,36]
...
@torch.no_grad()
def eval_split(model: HandJSNGateRegModel, loader: DataLoader) -> Dict[str, float]:
...
# 8) Train
# =========================
def train_epoch(model: HandJSNGateRegModel,
                loader: DataLoader,
...
# =========================
def main():
    log("🚀 CATCH | HAND-LEVEL | TASK=JSN regression | warmstart hand JSN binary ckpt | pred = sigmoid(gate)*36*sigmoid(reg) | best by Val(SOFT) PCC | + Conformal")

    df_hand = build_hand_level_df_jsn()
    if df_hand.empty:
        raise RuntimeError("No valid hand-level JSN images found.")
    ...
    train_ds = HandJSNDataset(df_tr, WHOLE_IMG_ROOT, train_tf)
    val_ds = HandJSNDataset(df_va, WHOLE_IMG_ROOT, eval_tf)
    test_ds = HandJSNDataset(df_te, WHOLE_IMG_ROOT, eval_tf)
    ...
    model = HandJSNGateRegModel(encoder).to(DEVICE)

    # warmstart
    if BINARY_CKPT_PATH and os.path.exists(BINARY_CKPT_PATH):
        warmstart_from_hand_jsn_binary_ckpt(model, BINARY_CKPT_PATH)
    else:
    ...
    reg_loss_fn = nn.SmoothL1Loss(reduction="mean")   # stable under skew
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        if np.isfinite(curr_val_pcc) and curr_val_pcc > best_val_pcc:
            ...
            save_name = f"catch_hand_jsn_warmstart_reg_bestValSoftPCC{best_val_pcc:.4f}_ep{ep}.pt"
            best_ckpt_path = os.path.join(MODEL_SAVE_DIR, save_name)
            torch.save(
                {
                    "model": model.state_dict(),
                    "task": "jsn_hand_regression",
                    "epoch": ep,
                    ...
                    "config": {
                        ...
                        "PRED": "sigmoid(gate) * (36 * sigmoid(reg_raw))",
