...
# -*- coding: utf-8 -*-
"""
Joint-level SvH Score Prediction (Erosion only in current setting)
Multi-seed version
"""
...
class VisionEncoder(nn.Module):
    """
    - AutoImageProcessor handles normalization/resize
    - Remove DINOv3 register tokens, keep [CLS+patches]
    """
...
def log_split_metrics(title: str, m_e: Dict[str, float]):
    log(
        f"📊 {title}\n"
        f"  Erosion: RMSE={m_e['rmse']:.3f}, R2={m_e['r2']:.3f}, PCC={m_e['pcc']:.3f}, MAE={m_e['mae']:.3f}"
    )

# =========================
# Train / Eval
# =========================
def train_epoch(model, loader, optimizer, loss_e) -> float:
    model.train()
    total, n = 0.0, 0
    for img, labels in loader:
        ...
        pred_e, _ = model(img)
        loss = loss_e(pred_e, eros_t, mask_e)
        loss.backward()
        optimizer.step()
...
def eval_loader(model, loader) -> Dict[str, Dict[str, float]]:
    ...
    for img, labels in loader:
        ...
        pred_e_list.append(pred_e.detach().cpu().numpy())
        tgt_e_list.append(eros_t.detach().cpu().numpy())

    pred_e = np.concatenate(pred_e_list, axis=0)
    tgt_e  = np.concatenate(tgt_e_list, axis=0)

    m_e = compute_metrics(pred_e, tgt_e)
    return {"erosion": m_e}
...
def run_one_seed(seed: int):
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        train_loss = train_epoch(model, train_ld, optimizer, loss_e)

        train_metrics = eval_loader(model, train_ld)
        tr_e = train_metrics["erosion"]
        log_split_metrics(f"Train (seed={seed}, epoch={ep})", tr_e)

        val_metrics = eval_loader(model, val_ld)
        va_e = val_metrics["erosion"]
        log_split_metrics(f"Val (seed={seed}, epoch={ep})", va_e)
        ...
        if val_pcc > best_val_pcc:
            ...
            best_val_metrics = va_e.copy()
            best_epoch = ep

            save_path = os.path.join(
                MODEL_SAVE_DIR,
                f"seed{seed}_airi_svh_joint_erosion_ep{ep}_valpcc{best_val_pcc:.4f}.pt"
            )
    ...
    test_e = test_metrics["erosion"]
    log_split_metrics(f"Test (seed={seed}, best_epoch={best_epoch})", test_e)

    log(f"Finished seed {seed} | Best epoch={best_epoch} | Best val metrics={best_val_metrics} | Test metrics={test_e}")
    return {
        "seed": seed,
